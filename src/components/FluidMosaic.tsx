// FluidMosaic.tsx
// Ported from the reference bento.html prototype: a nested flexbox tree where
// hovering (or tapping, on touch) a tile grows it AND every row/column that
// contains it, while siblings fluidly shrink — via spring-animated flex-grow.
'use client'

import React, { useState } from "react";
import Image from "next/image";
import { motion } from "framer-motion";
import "@/styles/FluidMosaic.css";
...
/** Spring-animated flex-grow container, using framer-motion — same spring
 * config (stiffness 280, damping 30, mass 1) as the bento.html reference. */
function SpringFlex({
  grow,
  className,
  style,
...
function Tile({
  ...
  hovered: string | null;
  onHover: (updater: string | null | ((h: string | null) => string | null)) => void;
}) {
  return (
    <SpringFlex
      ...
      onTouchStart={() => onHover((h) => (h === node.key ? null : node.key))}
    >
      <Image
        src={node.src}
        alt="#"
...
export default function FluidMosaic({ images }: { images: string[] }) {
  ...
  const { layout, info } = React.useMemo(() => buildLayout(images), [images]);

  return (
    <div className="fluidMosaicWrapper">
      <div className="fluidMosaicStage" onMouseLeave={() => setHovered(null)}>
